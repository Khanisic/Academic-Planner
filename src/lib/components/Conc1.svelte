...
<div class="flex flex-wrap gap-2">
	{#each program.program_concentrations as concentrationSelect}
		<button
			...
			class={`${concentration1 == concentrationSelect.concentration_name ? ' border-purple bg-purple' : 'bg-lightpurple'} border-2 border-lightpurple  shadow-md font-base hover:bg-purple rounded-lg px-3 cursor-pointer hover:text-navy`}
		>
	...
	<div class="flex flex-col gap-2">
		...
		{#if concentration1Details.concentration_elective_courses}
			...
			{#each concentration1Details.concentration_elective_courses as courses, index}
				<button
					on:click={() => {
						addConc1Course(`${courses.course_code} : ${courses.course_title}`, hoursConc1);
					}}
					class={`${selectedConc1Courses[`${courses.course_code} : ${courses.course_title}`] == true ? 'bg-purple' : 'bg-lightpurple'} border-2 border-lightpurple max-w-[350px] shadow-md font-base hover:bg-purple rounded-lg px-3 cursor-pointer `}
					>{courses.course_code} {courses.course_title}</button
				>
			{/each}
		{/if}
